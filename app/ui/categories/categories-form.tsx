...
export default function CategoryForm({ category }: Props ) {
  ...
  const file: MediaFiles = {id: 0, name: 'placeholder', path: category ? category?.image : '', disabled: false, mime_type: '', article_id: 0}
  const mediaFiles: [MediaFiles] | [] = category && category.image ? [file] : []; 
  ...
  return (
    <form action={formAction}>
      <div className="rounded-md bg-gray-50 p-4 md:p-6">
        ...
        {category ? <input type='hidden' value={category.id} id="category_id" name='category_id'/> : null}
        ...
        <FileChooser id='category-images'
          maxFilesAmount={1}
