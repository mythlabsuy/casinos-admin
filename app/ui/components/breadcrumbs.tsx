...
export default function Breadcrumbs({ breadcrumbs, homeUrl = '/' }: Props) {
  return (
    <nav aria-label="Breadcrumb" className='flex flex-row pr-4 mb-4'>
      ...
      {breadcrumbs.map((breadcrumb) => (
        <div key={breadcrumb.label} className='flex flex-row items-center' aria-current={breadcrumb.active}>
          ...
          <Link href={breadcrumb.href}>
            <h2 className={clsx(breadcrumb.active ? 'underline' : 'text-gray-800', "text-md font-bold tracking-tight")}>{breadcrumb.label}</h2>
          </Link>
        </div>
      ))}
    </nav>
