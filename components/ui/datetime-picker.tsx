...
export function DateTimePicker({
  ...
  const dislayFormat = useMemo(() => {
    if (!displayValue) return 'Selecciona una fecha';
    return format(
      displayValue,
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          {renderTrigger ? (
            ...
            <div
              className={cn(
                'border-input flex h-9 w-full cursor-pointer items-center rounded-md border bg-white pe-1 ps-3 text-sm font-normal shadow-sm',
...
function MonthYearPicker({
  ...
  const months = useMemo(() => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      months.push({
        value: i,
        label: format(new Date(0, i), 'MMM', { locale: es }),
        disabled,
      });
    }
    return months;
  ...
  return (
    <div className={cn(className)}>
      <ScrollArea className="h-full">
        {mode === 'year' && (
          <div className="grid grid-cols-4 rounded-sm bg-white">
            {years.map((year) => (
              <div
                key={year.value}
